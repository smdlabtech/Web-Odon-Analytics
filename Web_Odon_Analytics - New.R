...
ui <- shinyUI(
  ...
  dashboardPage(skin="green",   #skin: pour changer la changer la couleur du tableau de bord
                ...
                dashboardHeader(title = "Main dashbord "),
                ...
                dashboardBody(navbarPage(
                  ...
                  tabPanel("Database",icon = icon("database"),
                           ...
                           fluidRow(
                             ...
                             sidebarLayout(#-- Panneau lateral pour les entrees
                               sidebarPanel(# titlePanel("Upload Files")
                                 width = 3,
                                 ...
                                 #Selectionner un type de separateur
                                 radioButtons("sep", "Separator",choices = c(Comma = ","),selected = ","),
                                 
                                 #Selectionner le type de guillemets a ajouter
                                 radioButtons("quote", "Quote",choices = c("Double Quote" = '"'), selected = '"'),
                                 
                                 #ligne horizontal
                                 tags$hr(),
                                 
                                 #Selectionner le nombre de ligne a afficher
                               ...
                               mainPanel(tableOutput("contents"),
                                         #Affichage de la table de Choose CSV Data File
                                         column(1),
                                         column(10, reactableOutput("table1")),
                                         column(1))
                             )
                           )
                  ...
                  tabPanel("Interactive Overview",icon = icon("bar-chart-o"),
                           fluidPage(
                             ...
                             #Interaction Temp et graphiques (TempOd1,...,TempOd5)
                             absolutePanel(top = 130, 
                                           left = 900,
                                           selectInput("Temp",label = "Select a probe:",c("TempOd1", "TempOd2", "TempOd4", "TempOd5"))),
                             
                             
                             #-- Mise en forme de l'affichage graphique --
                             fluidRow(column(1),
                                      box(width=5,plotOutput('leplot6'),downloadButton('save_leplot6','')),
                                      box(width=5,plotOutput('leplot7'),downloadButton('save_leplot7',''))
                                      
                                      #box(width=5,plotOutput('plot5',height = 300),downloadButton('save_plot5','')),
                             ) #End fluidRow
...
server<-function(input, output, session) {
  ...
  output$contents <- renderPrint({
    ...
    if(input$disp == "head") {
      #requete resume statistiques
      resume_stats<-data.frame(matrix(df1 %>%
                                        summarise_each(funs(min,mean,median,max, sd), TempOd1,TempOd2, TempOd4,TempOd5),nrow = 4,ncol = 5))
      ...
      #Arrondir la table df1 de sortie
      ...
      output$daterange <- renderUI({
        dateRangeInput(inputId = "daterange",label = "Select a date range",
                       start = min(df1$date),   # The initial start date. Either a Date object, or a string in yyyy-mm-dd format
                       end = max(df1$date),     # The initial end date. Either a Date object, or a string in yyyy-mm-dd format 
                       min = min(df1$date),     # The minimum allowed date. Either a Date object, or a string in yyyy-mm-dd format.
                       max = max(df1$date),     # The maximum allowed date. Either a Date object, or a string in yyyy-mm-dd format.
    ...
    output$histplot1<-renderPlot({
      plot(as.numeric(df1$TempOd1)~as.Date(df1$date), type="l", col="red", xlab="Date", ylab = "Températures",main="Températures du site 812 entre 2011 et 2017")
    })
    ...
    output$histplot2<-renderPlot({
      plot(as.numeric(df1$TempOd2)~as.Date(df1$date), type="l", col="blue", xlab="Date", ylab = "Températures",main="Températures du site 813 entre 2011 et 2017")
    })
    ...
    output$histplot4<-renderPlot({
      #hist(df1$TempOd4,main = "Histogramme des Temp d'Odon T4", xlab = "Variations des Temp")
      plot(as.numeric(df1$TempOd4)~as.Date(df1$date), type="l", col="green", xlab="Date", ylab = "Températures",main="Températures du site 815 entre 2011 et 2017")
    })
    ...
    output$histplot5<-renderPlot({
      #hist(df1$TempOd5,main = "Histogramme des Temp d'Odon T5", xlab = "Variations des Temp")
      plot(as.numeric(df1$TempOd5)~as.Date(df1$date), type="l", col="yellow", xlab="Date", ylab = "Températures",main="Températures du site 816 entre 2011 et 2017")
    })
    ...
    output$leplot6 <- renderPlot({
      plot(as.numeric(df1[[input$Temp]])~as.Date(df1$date), type="l", col="red", xlab="Date", ylab = "Températures",main=paste("Températures de ", input$Temp,"entre 2011 et 2017"))
    })
    ...
    output$save_leplot6<-downloadHandler(
      ...
      content=function(file){
        png(file)
        plot(as.numeric(df1[[input$Temp]])~as.Date(df1$date), type="l", col="red", xlab="Date", ylab = "Temp",main=paste("Températures de ", input$Temp,"entre 2011 et 2017"))
        dev.off()
      }
    ...
    # #leplot6 et leplot7
    # #Affichage graphques
    # # plot(as.numeric(df1$TempOd1)~Input_date(), type="l", col="yellow", xlab="Date", ylab = "Températures",main="Températures entre 2011 et 2017")
    # output$leplot6 <- renderPlot({
    #   plot(dataInput(), type="l", col="yellow", xlab="Date", ylab = "Températures",main="Températures entre 2011 et 2017")
    # })
  ...
  output$Mapcontents <- renderPrint({
    ...
    if(input$disp == "head") {
      ...
      #Interpretation MAP
      #Sortie par d'info du cote serveur(Affichage de la carte)
